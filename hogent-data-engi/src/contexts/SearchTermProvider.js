...
export const SearchTermProvider = ({
  ...
  const addSearchterm = useCallback(async (term, parent) => {
    try {
      setLoading(true);
      setError();
      ...
      if (parent) {
        await api.addSearchtermWithParent(term, parent);
      } else {
        await api.addSearchtermWithoutParent(term);
      }
  ...
  const deleteSearchterm = useCallback(async (id) => {
    try {
      setLoading(true);
      setError();

      await api.deleteSearchterm(id);
